...
INNER JOIN doctor d ON i.doctor_id = d.doctor_id where invoice_number = "' .$invoice_number. '"';
	...
	public function getDoctorDailySales($doctor_id, $selected_date)
	{
		$pcon = $this->getConnection();
		$this->selectDB($pcon);
	...
	public function getdailyreceptionsale($login, $selected_date)
	{
		...
		$sql = "SELECT p.patient_reg_no, p.name, i.createdby, i.careof, i.creation_time, SUM(recievedamount) as receivedamount FROM invoice i
INNER JOIN patient p ON i.patient_id = p.patient_id WHERE DATE_FORMAT(i.creation_time, '%m/%d/%Y') = '$selected_date' 
...
INNER JOIN patient p ON ba.patient_id = p.patient_id WHERE p.patient_id =$pat_id ";
		$RS = mysql_query($sql);
		$RC = mysql_num_rows($RS);
		if ($RC > 0)
		{			
