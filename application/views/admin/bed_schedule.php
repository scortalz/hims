...
<input type="button" style="float:right;margin-left:10px;" name="Report" id="Report" onclick="scheduleprint()" value="Print Report" class="btn btn-green" title="Click here to print" />

<input type="button" style="float:right;" name="BedSchedule" id="BedSchedule"  value="Update Bed Schedule" class="btn btn-green" title="Click here to print" />
...
//			{
	...
	if($i==1)
		...
		echo " <tr>";
			if($m==1)
				echo "<td>&nbsp;<strong>Bed Name</strong> </td>";	
			echo "<td  ><div align='center'><strong>".date('m/d/Y', strtotime($head_row_date))."</strong><br/></div></td></tr>";
		$head_row_date = date('m/d/Y',strtotime('+1 day',strtotime($head_row_date)));
			...
			echo '<tr >';
			if($m==1)
		echo '<td > <strong>'.$arrbedno['type'].'&nbsp;</strong>'.$arrbedno['bed_number'].' </td>';
		//else
		//echo '<td></td>';
			echo '<td>';
		...
		echo '</pre>';*/
			...
			foreach($arrBS as $arr) 
			{
				if($arr['bed_id']==$i )
				{
					echo "<span style='background-color:red;color:yellow;margin-left:8px;'><strong> ".$arr['name'];" </strong></span>";
...
<script>
	...
	jQuery("#BedSchedule").click(function()
	{		
		jQuery.ajax({
		type: "POST",
		url: "<?php echo base_url();?>/application/helpers/updatebedschedule.php",
			success: function(response)	
			{
				alert('Bed schedule has been successfully updated.');
				location.reload();
