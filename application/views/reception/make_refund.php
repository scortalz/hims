    ...
    <tr>
     ...
     <td>
    	<label><b><?php echo get_phrase('Date');?></b></label>
           <input type="text"  name="refund_date" id="refund_date" class="datepicker fill-up" />
       </td>
       </tr>
    
    	<tr>
	
				<td><label class="total"><b>Total Refund:</b></label>         
				
            <input type="text" id="txtrefund" name="txtrefund"  style="width: 100px;text-align:right;padding-right:10px;"/>
                 </td>   
			</tr>
...
<br />
    ...
    <div style="float:left;">
	  	<input type="submit" id="btnSaveRefund" name="btnSaveRefund" value="   Save  " class="btn btn-green"/>
	  </div>
	...
	$("#txtrefund").keydown(function(event) {
		//alert(event.keyCode);
	...
	$('#btnSaveRefund').click(function(){  	
		...
		if ( $('#txtrefund').val() < 0 )
		{
			...
			alert('Refund Amount not allowed less than 0');
			$('#txtrefund').focus();
		  	return false;
		}
		
		// End Validation Contact Information..
		var confirmLeave = confirm('Are you sure you want to save refund amount ?');
		...
		var refund_amount		= $('#txtrefund').val();
		
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url();?>application/helpers/postrefund.php",
			data: ({ 
				post_reg_no 			: reg_no,  
				post_refund_date 		: refund_date,
				post_refund_amount 		: refund_amount,
			}),
			success: function(response)	
			{
				alert('Refund has been successfully posted');
			}
		});	
	});
