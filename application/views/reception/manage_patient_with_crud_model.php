		...
		<div class="tab-content" >
			...
			<div class="tab-pane box active" id="edit" style="padding: 5px">
                <div class="box-content">
                    ...
                    <?php echo form_open('reception/manage_patient/edit/do_update/'.$row['patient_id'] , array('class' => 'form-horizontal validatable'));?>
                        <div class="padded">
						...
						<input type="text" required="required" name="med_card_no" id="med_card_no" value="<?php echo $row['med_card_no'];?>" style="display:none;width:13%"  />
				...
				<!--	<td style="text-align:center;padding: 4px 2px 1px 4px; border: 1px solid; border-color: #0000;">'.$row['age'].'</td>
					...
					'. $row['discharge_type'].'</td>
                        ...
                        <tr>
							...
							<?php /*?><td><?php echo $row['refferedby'];?></td><?php */?>
		...
		<input type="checkbox"  name="sameasabove" id="sameasabove" /><span id="labelsameabove" style="vertical-align:middle">&nbsp;Same as above</span>
				...
				<input type="text" name="end_1" id="end_1" placeholder="1"  maxlength="1" style="width:2%"/>
                            ...
                            <div class="control-group">
                                <label class="control-label"><?php echo get_phrase('phone');?></label>
                                <div class="controls">
                                    <input type="text" class="validate[required]" name="phone" id="phone" maxlength="12"/>
                                </div>
                            </div>
                            <div class="control-group">
                                <label class="control-label"><?php echo get_phrase('sex');?></label>
                            ...
                            <!--<div class="control-group">
                                <label class="control-label"><?php echo get_phrase('birth_date');?></label>
                                <div class="controls">
                                    <input type="text" required="required" name="birth_date" id="birth_date" />
                                </div>
                            </div>-->
                            <div class="control-group">
                                <label class="control-label"><?php echo get_phrase('age');?></label>
...
/*	var charges='';
	...
	{
		var roomtype_id = $("select[name='room_id']").val();
		//alert(roomtype_id);
		$('#roomcharges').val();
		...
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url();?>/application/helpers/getroomcharges.php",
			data: ({post_roomtype_id: roomtype_id}),
			success: function(response)	
			{
			//	alert(response);
			charges=response;
				$("#roomcharges").val(response);
				$('#days').val('');
			}
		});	
	  ...
	  $('#days').blur(function(){
		var roomcharges  = charges;//$('#roomcharges').val();
		var days  = $('#days').val();
		
		//alert(days);
		var totalroomcharges = parseInt(parseInt(roomcharges) * parseInt(days));//.toFixed(2);
			
			$('#roomcharges').val(totalroomcharges);
			//alert($('#dueamount').val());
 	});*/
	...
	$("#email").blur(function(){
		...
		$("#salutation").change(function(){
			...
			{
				$("#sex").val('Female');
				$("select#sex").prop('selectedIndex', 1);
			}
			else
			{
				$("#sex").val('Male');
