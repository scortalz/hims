		...
		<ul class="nav nav-tabs nav-tabs-left">
			...
			<li>
            	<a href="#add" data-toggle="tab"><i class="icon-plus"></i>
					<?php echo get_phrase('add_advance_appointment');?>
                    	</a></li>
		</ul>
    	<!------CONTROL TABS END------->
		...
		<div class="tab-content">
                ...
                <div class="box-content">
                    ...
                    <?php echo form_open('reception/advanceappointment/edit/do_update/'.$row['app_id'] , array('class' => 'form-horizontal validatable'));?>
                        <div class="padded">
                                ...
                                <label class="control-label"><?php echo get_phrase('patname');?></label>
                                <div class="controls">
           ...
           <div class="tab-pane box <?php if(!isset($edit_profile))echo 'active';?>" id="list">
            ...
            -->
              ...
              <?php if($advance_appointment_data->num_rows() > 0 ){
             $count = 1; foreach($advance_appointment_data->result_array() as $row){ ?>
                        <tr>
               <td><?php echo  $count++;?></td>
               <td><?php echo  $row['invoice_number'];?></td>
               <td><?php echo  $row['totalamount'];?></td>
               <?php $ans =  $this->db->get_where('patient',array('patient_id'=>$row['patient_id']))->row();

               //print_r($ans);die;?>
               <td><?php echo  $ans->name;?></td>
               <td><?php echo  $ans->phone;?></td>
                 <?php $mydata =  $this->db->get_where('doctor',array('doctor_id'=>$row['doctor_id']))->row(); ?>
               <td><?php echo  $mydata->name;?></td>

               <td><?php
                //$dateTom = $row['date_of_appointment']  >= CURDATE();
              //  echo $dateTom;
...
<div class="box-content">
  ...
  <?php echo form_open('reception/advanceappointment/create' , array('class' => 'form-horizontal novalidate','id' => 'advapt'));?>
  ...
  <input type="hidden"  name="createdby" id="createdby" readonly="readonly" value="<?php echo $this->crud_model->get_type_name_by_id($account_type , $this->session->userdata($account_id) , 'name'); ?>"/>
      ...
      <input type="hidden" id="conDisc" name="conDisc" value="0" />
       <div class="control-group">
              <label class="control-label"><?php echo get_phrase('date_of_appointment');?></label>
              <div class="controls">
       <!--  date ki input -->     <input type="date" class=" " name="date_of_appointment" id="doa" value="" min="<?php echo date("Y-m-d"); ?>"/>
              </div>
          </div>
      <div class="control-group">
...
<div class="box-content" id="add_patient" style="display:none;">
 ...
 <form method="post" action="<?php echo base_url();?>index.php?" id="advapt" class="form-horizontal validatable">
     <div class="padded">
           <div class="control-group">
...
<!--         <div class="control-group">
              ...
              <div class="controls">
                  <input type="date" class="  fill-up" name="creation_timestamp" min="<?php echo date("Y-m-d"); ?>" value=""/>
              </div>
          </div> -->
          <?php /*?><div class="control-group">
              <label class="control-label"><?php echo get_phrase('status');?></label>
...
$( "#advapt" ).submit(function( event ) {
      ...
      if(date == ''){
        $('#error').show();
        $('#error').html('');
        $('#error').html('please Select the date of appointment');
        event.preventDefault();
        return;
      }
      $('#error').hide();
