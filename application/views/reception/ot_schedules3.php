		...
		<ul class="nav nav-tabs nav-tabs-left">
			...
			<li>
            	<a href="#OT2" data-toggle="tab"><i class="icon-align-justify"></i>
					<?php echo get_phrase('OT_Schedule3');?>
                    	</a></li>
		</ul>
    	<!------CONTROL TABS END------->
...
<div class="container-fluid padded">
    ...
    font-weight: 600;">
                    <div class="title">
                        <i class="icon-calendar"></i> <?php echo get_phrase('OT Schedules3 - 24 Hours') . " -  ".date('D d M Y', strtotime($current_date)) ;?>
                    </div>
                </div>
				         ...
				         for ($i = 0; $i < 48; $i++) 
    				       {
						    if (count($arrRegNo1) > 0 )
						     {
						      $arrRegNo = $db->GetOTSchedules3(date("H:i",$iTimestamp), $current_date);
...
( $iTimestamp + 1800 )). " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p>\n<br /></div>"; 
	...
	//alert(data)
		 jQuery.ajax({
		type: "POST",
		url: "<?php echo base_url();?>/application/helpers/printotschedule3.php",
		data: ({post_date_id: data}),
			success: function(response)	
			{
				//alert(response);
				var v = "reports/otschedule3.pdf";
				 window.location = v;
