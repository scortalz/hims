    ...
    <tr>
     ...
     <td>
    	<label><b><?php echo get_phrase('Date');?></b></label>
           <input type="text"  name="zakaat_date" id="zakaat_date" class="datepicker fill-up" />
       </td>
       </tr>
    
    	<tr>
	
				<td><label class="total"><b>Total Zakaat Amount:</b></label>         
				
            <input type="text" id="txtzakaat" name="txtzakaat"  style="width: 100px;text-align:right;padding-right:10px;"/>
                 </td>   
			</tr>
...
<br />
    ...
    <div style="float:left;">
	  	<input type="submit" id="btnSaveZakaat" name="btnSaveZakaat" value="   Save  " class="btn btn-green"/>
	  </div>
	...
	$("#txtzakaat").keydown(function(event) {
		//alert(event.keyCode);
	...
	$('#btnSaveZakaat').click(function(){  	
		//alert ($('#txtdollarrate').val());
		
		if ( $('#txtzakaat').val() < 0 )
		{
			
			alert('Zakaat amount not allowed less than 0');
			$('#txtzakaat').focus();
		  	return false;
		}
		
		// End Validation Contact Information..
		var confirmLeave = confirm('Are you sure you want to save zakaat amount ?');
		...
		var zakaat_date 		= $('#zakaat_date').val();
		var zakaat_amount		= $('#txtzakaat').val();
		
		jQuery.ajax({
			type: "POST",
			url: "<?php echo base_url();?>application/helpers/postzakaat.php",
			data: ({ 
				post_reg_no 			: reg_no,  
				post_zakaat_date 	: zakaat_date,
				post_zakaat_amount 	: zakaat_amount,
			}),
			success: function(response)	
			{
				alert('Zakaat has been successfully posted');
			}
		});	
	});
