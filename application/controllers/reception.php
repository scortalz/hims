...
class Reception extends CI_Controller
{
	...
	public function get_search_data(){
		...
		$search_data = $this->input->post('name');
	   
	    $patient_data = 	
	    $this->db->where("(	patient_reg_no='".$search_data."' OR name='".$search_data."')", NULL, FALSE)->get('patient');
	     
	    //print_r($data);die;
	   if($patient_data->num_rows() > 0){ 
	   
	   	   $patient_id = $patient_data->row()->patient_id;
	    	$invoice_data = $this->db->get_where('invoice',array('patient_id'=>$patient_id) );
	    	 $this->db->last_query();
	    	//print_r($invoice_data);die;
	   	if($invoice_data->num_rows()>0){
	   	   
            $page_data['invoice_data']= $invoice_data->row();
	    	$doctor_id = $invoice_data->row()->doctor_id;


	    	
            $doctor_data = $this->db->get_where('doctor',array('doctor_id'=>$doctor_id))->row();


        $page_data['doctor_id']= $doctor_data;     
        
	   	}
	   	// print_r($result);die;
	   	$page_data['search_result'] = $patient_data->row() ;
	   	
	     $page_data['page_name']  = 'search_patient_data';
         $page_data['page_title'] = get_phrase('registered_patient');
         
        $this->load->view('index', $page_data);
	}
	elseif (isset($_POST['submit'])) {
		$search_data = $this->input->post('patname');
	   $data = 	$this->db->get_where('advappointment',array('patname'=>$search_data));
	   //print_r($data);die;
	   if($data->num_rows() > 0){ 
	   	$page_data['search_result'] = $data->row() ;
	     $page_data['page_name']  = 'search_advappoinment';
         $page_data['page_title'] = get_phrase('advanceappointment');
		# code...
         $this->load->view('index', $page_data);
	}
	/*elseif (isset($_POST['submit'])) {
		$search_data = $this->input->post('');
	   $data = 	$this->db->get_where('Appointment',array('appointment_id'=>$search_data));
	  // print_r($data);die;
	   if($data->num_rows() > 0){ 
	   	$page_data['search_result'] = $data->row() ;
 ...
 function manage_patient($param1 = '', $param2 = '', $param3 = '')
 {
  ...
  /* $reg_no = 'MR-'.date('ym-', time()).str_pad($id, 4, '0', STR_PAD_LEFT);
   ...
   redirect(base_url() . 'index.php?reception/manage_patient_slip/'.$id);
 ...
 function manage_daily_expense($param1 = '', $param2 = '', $param3 = '')
 {
  if ($this->session->userdata('reception_login') != 1)
   redirect(base_url() . 'index.php?login', 'refresh');
  ...
  if ($param1 == 'create') {
            ...
            $data['receptionest_name']       =  $this->session->userdata('reception_name');
			$data['reason']                  = $this->input->post('Reason');
			$data['expense_amount']          = $this->input->post('expense_Amount');
			$data['date']                    = $this->input->post('date');
   $this->db->insert('expense_voucher', $data);
  ...
  /* $reg_no = 'MR-'.date('ym-', time()).str_pad($id, 4, '0', STR_PAD_LEFT);
   ...
   redirect(base_url() . 'index.php?reception/manage_daily_expense/'.$id);
  ...
  if ($param1 == 'edit' && $param2 == 'do_update') {
			...
			$data['ACCOUNT_Id']       = $this->input->post('respetion_name');
			$data['reason']             = $this->input->post('reason');
			$data['expense_amount']          = $this->input->post('expense');
			$data['date']            = $this->input->post('date');
			
			
			$this->db->where('expense_voucher', $param3);
			$this->db->update('expense_voucher', $data);
			$this->session->set_flashdata('flash_message', get_phrase('account_updated'));
			
			redirect(base_url() . 'index.php?reception/expense_voucher', 'refresh');
		} else if ($param1 == 'edit') {
			$page_data['edit_profile'] = $this->db->get_where('expense_voucher', array(
				'expense_voucher' => $param2
			))->result_array();
		}
  
  $page_data['page_name']  = 'expense_voucher';
  $page_data['page_title'] = get_phrase('expense_voucher');
	//$this->db->order_by("patient_reg_no","desc");  /*/this line added by ali/*/
  $page_data['expense_voucher']   = $this->db->get('expense_voucher')->result_array();
   
  $this->load->view('index', $page_data);
 }


 	public function manage_patient_slip($id) {
      
    $this->db->select('p.name,p.patient_reg_no,p.patient_type,p.nic_no,p.sex,p.age,p.phone,p.admission_date,d.name as doctor_name');
    $this->db->from('patient as p');
    $this->db->join('doctor as d','d.doctor_id = p.doctor_id','left');
    $this->db->where('patient_id',$id);
     $result = $this->db->get();
    $data['result'] = $result;
 	$this->load->view('patient_slip',$data);	
 	}

     	 /***Discharge patients**/
	public function Manage_Shift_Closed($param1 = '', $param2 = '', $param3 = '')
	{
     // echo "my shift closed controller";
     // exit();

		// echo $this->session->userdata('reception_name');
		// exit();
		if ($this->session->userdata('reception_login') != 1)
             redirect(base_url() . 'index.php?login', 'refresh');
		...
		if ($param1 == 'Create_Shift_Closed') {
			...
			$data['reason']    = $this->input->post('reason');
			$data['admin_id']    = $this->session->userdata('reception_id');
			$data['name'] =       $this->session->userdata('reception_name');
			$data['Closing_Date'] =   date ('Y/m/d');
		    // $this->db->select('re.name as receptionname');
            // $this->db->from('reception re');
            // $this->db->join('Manage_Shift_Closed msc', ' $checkingvar = re.reception_id');
            // $this->db->where(array('re.reception_id'=>$this->session->userdata('reception_id')));
            // $managedata = $this->db->get();
            // $data = $data->result();
 			// $abc = $managedata->row()->receptionname;  
			...
			// $this->email_model->account_opening_email('laboratorist', $data['email']); //SEND EMAIL ACCOUNT OPENING EMAIL
			 $this->session->set_flashdata('flash_message', get_phrase('Shift_Closed'));
			  redirect(base_url() . 'index.php?reception/Manage_Shift_Closed', 'refresh');
		}
		if ($param1 == 'edit' && $param2 == 'do_update') {
			$data['Cash_Recieved']     = $this->input->post('Cash_Recieved');
			$data['Total_bill']    = $this->input->post('Total_bill');
  ...
  //      $page_data['Shift_Closed'] = $this->db->get()->result_array();
        $admin="admin";
         $page_data['Shift_Closed']= $this->db->get_where('Manage_Shift_Closed',array('name !='=>$admin))->result_array();
         // print_r($ans);die;
...
/***DEPARTMENTS OF DOCTORS********/
			 ...
			 if(isset($_POST['check_box']))
			 	{
			 		...
			 		$this->mdl_invoice->add_patient_also_invoice();
			 		// print_r($ans);die;
			 		
			    }else
			    {
			    	$data = $this->mdl_invoice->add_invoice_only();
			    }
		
			/*$this->load->model('mdl_invoice');
		    $this->mdl_invoice->add_invoice();*/
			
			
			$this->session->set_flashdata('flash_message', get_phrase('invoice_created'));
			redirect(base_url() . 'index.php?reception/manage_invoice_report/'.$this->input->post('invoice_no'));
    ...
    {
     $this->db->select('ism.service_amount,d.name as service_name');
    ...
    $this->db->select('i.invoice_number,i.totalamount,i.discountamount,i.recievedamount,i.dueamount,i.creation_time,i.createdby,p.patient_reg_no,
     ...
     $this->load->view('patient_invoice_slip',$data);
...
*/
 ...
 /******MANAGE floor*****/
	...
	function ipdaddservice($param1 = '', $param2 = '', $param3 = '')
	{
	 ...
	 function generate_ipd_invoice($reg_no)
	 {
	  $this->db->select_max('id');
      $query = $this->db->get('patient_services')->row();
	  $this->db->select('ps.id,ps.patient_reg_no,ps.service_start_date,ps.service_amount,ps.bed_no,ps.service_total_amount,d.name as doctor_name,p.name as patient_name,p.father_husbandname,p.address,
	  	                 p.phone,p.sex,p.age,ds.name as service_name');
	  $this->db->from('patient_services as ps');
	  $this->db->join('doctor as d','d.doctor_id = ps.doctor_id','left');
	  $this->db->join('patient as p','p.patient_id=ps.patient_id','left');
	  $this->db->join('diagnosticservice as ds','ds.diagnosticservice_id = ps.service_id','left');
      $this->db->where('ps.id',$query->id);
      
      $my_data= $this->db->get();
      // print_r($my_data);die;
      $data['ipd_invoice_data']= $my_data;
	...
	*/
	...
	/*	$data = array('status' =>0);
	 ...
	 /******IPD Whole Discount ****/
	function make_discount($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('reception_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
	
		$page_data['page_name']  = 'make_discount';
		$page_data['page_title'] = get_phrase('Discount');
		
		$this->load->view('index', $page_data);
	}
	
		 /******IPD Whole Refund ****/
	function make_refund($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('reception_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
	
		$page_data['page_name']  = 'make_refund';
	 ...
	 /******IPD Whole Zakaat ****/
	function make_zakaat($param1 = '', $param2 = '', $param3 = '')
	{
		if ($this->session->userdata('reception_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');
	
		$page_data['page_name']  = 'make_zakaat';
		$page_data['page_title'] = get_phrase('Zakaat');
	...
	function advanceappointment($param1 = '', $param2 = '', $param3 = '')
	{
			...
			if(isset($_POST['check_box']))
			 	{
			 		...
			 		$this->mdl_invoice->add_patient_also_appointment();
			 		// print_r($ans);die;
			 		
			    }else
			    {
			    	$data = $this->mdl_invoice->add_advance_appointment_invoice_only();
			    	
			    }
			// $this->db->insert('advappointment', $data);
			$this->session->set_flashdata('flash_message', get_phrase('advanceppointment_created'));
			redirect(base_url() . 'index.php?reception/manage_invoice_report/'.$this->input->post('invoice_no'));
		}
		...
		if ($param1 == 'edit' && $param2 == 'do_update') {
			...
			redirect(base_url() . 'index.php?reception/advanceappointment', 'refresh');
		...
		if ($param1 == 'delete') {
			...
			redirect(base_url() . 'index.php?reception/advanceppointment', 'refresh');
		}
		
	
		$page_data['page_name']  = 'advanceappointment';
		$page_data['page_title'] = get_phrase('advance_appointment');
		// $ans =  $this->db->get('my_advance_appointment')->result_array();
		
		$page_data['advance_appointment_data'] =  $this->db->query('SELECT * FROM my_advance_appointment WHERE date_of_appointment >= CURDATE()');
		$this->load->view('index', $page_data);
	}	  
	...
	function doctor_share_summary(){
		if ($this->session->userdata('reception_login') != 1)
			redirect(base_url() . 'index.php?login', 'refresh');

		$page_data['page_name']  = 'doctor_share_summary';
		$page_data['page_title'] = get_phrase('doctor_share_summary');
		
		
		$this->load->view('index', $page_data);
	}

	function doctor_share_summary_report(){
		$from = $this->input->post('date_from');
		$to = $this->input->post('date_to');
		$doctor_id = $this->input->post('doctor_id');
		
	
		  $where = '(CAST(patient.admission_date as DATE) between "'.$from.'"  AND  "'.$to.'" AND patient.doctor_id="'.$doctor_id.'")';
		 $page_data['patients']   = 
		$this->db
		->select('patient.*,doctor.ratio,patient_services.*')
		->from('patient')
		->where($where)
		->join('doctor', 'patient.doctor_id = doctor.doctor_id')
		->join('patient_services', 'patient.patient_reg_no = patient_services.patient_reg_no')
		->order_by("patient.patient_reg_no", "asc")
		->get()
		->result_array();
		
		
	  $this->load->view('reception/doctor_share_summary_slip',$page_data);
	}

	function f_e_v(){
	$from = $this->input->post('getit');
	$page_data['tbody'] = $from;
	  $this->load->view('reception/f_e_v',$page_data);
	}

 
/*   Invoice ki editing n save */
