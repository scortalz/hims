	...
	function _validate_login($str)
	{
		...
		if ($query->num_rows() > 0) {
			...
			if ($this->input->post('login_type') == 'laboratorist') {
				...
				$this->session->set_userdata('laboratorist_name', $row->name);
			}
			if ($this->input->post('login_type') == 'reception') {
				$this->session->set_userdata('login_type', 'reception');
				$this->session->set_userdata('reception_login', '1');
				$this->session->set_userdata('reception_id', $row->reception_id);
				$this->session->set_userdata('reception_name', $row->name);
			}
			return TRUE;
		} else {
			$this->session->set_flashdata('flash_message', get_phrase('login_failed'));
